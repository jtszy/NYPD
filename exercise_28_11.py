...
# now max_dist is calculated more optimally in dijkstra() function

def calc_connected_component_diameter(
    graph_edges: EdgeType, current_diameter: float, start: str
):
    distance_dict, previous_vertex, max_dist = dijkstra(graph_edges, start)
...
def dijkstra(graph_edges: EdgeType, start: str) -> Tuple[DistanceDict, PathInfo, int]:
    ...
    while que:
        ...
        for neigh_v, edge_len in neighbourhood[vertex]:
            if distance_dict[neigh_v] > dist + edge_len:
                ...
                max_dist = max(max_dist, dist + edge_len)
                add_element(que, distance_dict[neigh_v], neigh_v)
                previous_vertex[neigh_v] = vertex

    return distance_dict, previous_vertex, max_dist
